...
def prepare_safe_gate_inputs_gva(
    batch_size,
    T,
    HQK,
    HV,
    ...
    """Prepare native GVA inputs for cuLA chunk_kda (q/k stay in HQK head space).

    Unlike ``prepare_safe_gate_inputs`` which expands q/k to HV heads,
    this function returns q/k in the smaller HQK space so that cuLA can
    exercise its native GVA path (HQK < HV).

    GVA layout:
        q, k       : (batch_flat, T, HQK, D)  — l2-normalised, no grad
        v          : (batch_flat, T, HV,  D)
        g, beta    : (batch_flat, T, HV,  D / ∅)  — after kda_gate_chunk_cumsum
        A_log      : (HV,)
        dt_bias    : (HV * D,)
        init_state : (num_seqs, HV, D, D) or None

    The ``g`` tensor is the *pre-processed* chunk-cumsum gate, ready to be fed
    directly into chunk_kda with ``use_gate_in_kernel=True``.
    """
    ...
    q = torch.randn(batch_size, T, HQK, D, dtype=dtype, device=device).requires_grad_(False)
    k = torch.randn(batch_size, T, HQK, D, dtype=dtype, device=device).requires_grad_(False)
    v = torch.randn(batch_size, T, HV,  D, dtype=dtype, device=device).requires_grad_(False)
    g = torch.randn(batch_size, T, HV,  D, dtype=dtype, device=device).requires_grad_(False)
    beta = torch.randn(batch_size, T, HV, dtype=torch.float, device=device).sigmoid().requires_grad_(False)

    # l2-normalise q/k in the HQK head space (native GVA: one QK head per group).
    q, _ = l2norm_fwd(q)
    k, _ = l2norm_fwd(k)
